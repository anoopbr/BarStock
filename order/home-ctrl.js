(function() {
  ...
  angular.module('barstock')
    .controller('HomeCtrl', function($scope, $filter, $window, Order, UnitConversion, Stocks, Drinks) {
      $scope.stock = Stocks;
      $scope.drinks = Drinks;
      ...
      angular.forEach(filterList, function(filterString) {
        var filteredItems = getStocks(filterString);
      ...
      function getStocks(filters) {
        ...
        angular.forEach(filterStrings, function(filterString) {
          filteredStockItems = filteredStockItems.concat($scope.stock.filter(function(stockItem) {
            return stockItem.type === filterString;
          }));
        });
      ...
      function initDrinksOrder() {
        for (var index = 0; index < $scope.drinks.length; ++index) {
          $scope.drinksOrdered[index] = ({
            name: $scope.drinks[index].name,
            quantity: 0
          });
        }
      }
      ...
      $scope.getQtyAvailable = function(stockItem) {
      ...
      $scope.getIngredient = function(stockId) {
        switch (stockId) {
          case 9:
            return 'Lime';
          case 10:
        ...
        return $scope.stock.filter(function(stockItem) {
          return stockItem.stockId === stockId;
        })[0].name;
      };
      ...
      $scope.increaseDrinkCount = function(index, drink) {
        ...
        angular.forEach(drink.ingredients, function(ingredient) {
          var stockItem = $scope.stock.filter(function(stockItem) {
            return stockItem.stockId === ingredient.stockId;
          })[0];
          stockItem.usedQty =
        ...
        angular.forEach($scope.drinks, function(drink) {
          ...
          angular.forEach(drink.ingredients, function(ingredient) {
            ...
            var normalizedRequiredQty = ((ingredient.measure && stockItem.measure) ?
              unitConversion.convert(ingredient.qty, ingredient.measure, stockItem.measure) : ingredient.qty);
            if ($scope.getQtyAvailable(stockItem) < normalizedRequiredQty) {
  ...
  angular.module('barstock').factory('Stocks', function() {
  ...
  angular.module('barstock').factory('Drinks', function() {
